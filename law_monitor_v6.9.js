...
const CONFIG = {
     ...
     * 看板正文与运行日志里始终会附。
     */
    npcObserverInSingleNotify: false,
...
function buildSummaryBody(board) {
    ...
    if (consulting.length > 0) {
        ...
        consulting.forEach(function(it) {
            ...
            /*
             * V6.9：NPC Observer 对应条目页，另起一行附在下面
             */
            const npcUrl =
...
function buildReportUrl(items) {
    ...
    while (cap > 0) {
        ...
        const html =
            ...
            "<h2>⚖️ 中国法律监控 V6.9 · 本次新增 " + items.length + " 条</h2>" +
...
function sendNotification(item) {
    ...
    /*
     * V6.9：征求意见类可选附 NPC Observer 链接。
     * 默认关（CONFIG.npcObserverInSingleNotify=false），
    ...
    if (
        CONFIG.npcObserverInSingleNotify &&
        (
            category === "征求意见" ||
            category === "法律草案"
        )
    ) {
        npcUrl = findNpcObserverUrl(item);
    }
