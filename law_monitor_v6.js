...
function extractDeadline(title) {
     ...
     */
    const deadlineRe =
        /(截止|征询|反馈|期限|至)[^0-9]{0,12}(20\d{2})[-年.](\d{1,2})[-月.](\d{1,2})/;
...
async function main() {
    ...
    if (
        ...
        /*
         * 首跑防护（CG 建议）：
         * 若所有数据源网络都失败，得到的 allItems 必为空，
         * 此时若建立空历史，下次站点恢复会把所有真条目当"新增"狂推。
         * 故所有源失败时不初始化，提示用户检查网络/配置。
         */

        if (
            NET_FAIL_SOURCES.length ===
            CONFIG.sources.length
        ) {

            console.log(
                "首次运行：全部数据源访问失败，不初始化历史库"
            );
