...
const CONFIG = {
    ...
    /*
     * flcaw 调试开关（默认关）：
     * 打开后，每次抓到 flcaw 源时把【原始 HTML 响应】整段
     * 打印到 Loon 控制台日志（标记 DEBUG_FLCAP_RAW 开始/结束），
     * 用于定位"法律草案征求意见"抓取不到的真正原因
     * （静态 HTML 壳 / JS 渲染 / 接口路径）。
     * 确认修好后请设回 false，避免日志刷屏。
     */
    debugFlcaw: true,
...
function buildReportUrl(items) {
    ...
    while (cap > 0) {
        ...
        const html =
            ...
            "<h2>⚖️ 中国法律监控 V6.2 · 本次新增 " + items.length + " 条</h2>" +
